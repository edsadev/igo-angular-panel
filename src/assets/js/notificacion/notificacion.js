 ...
 *    @file ~/freelife/public/js/notificacion/notificacion.js
...
var Notificacion = function () {
    ...
    //cache DOM
    var $layout = $('div.notificacion');
    var $menu = $("#menu-notificacion");
...
//        var $form = $agregar.parents("form");
    ...
    function _get(event) {
        ...
        var resquest = ajax(config.pathname.notificacion.get, datos, beforeSend, HTTPMethod, true, 'application/x-www-form-urlencoded; charset=UTF-8', 'json', statusCode, true, headers);
            ...
            if (result.data.length > 0) {
                ...
                for (var key in result.data) {
                    ...
                    $dataTable.row.add([
                        ...
                        '<button type="button" class="btn btn-sm btn-icon btn-pure btn-outline delete-row-btn" data-toggle="tooltip" data-original-title="Delete" data-href="' + config.pathname.notificacion.delete + result.data[key].id + '"><i class="ti-close" aria-hidden="true"></i></button>'
...
//        var $form = $(event.target).parents("form");
        ...
        var resquest = ajax( $id.val() === '' ? config.pathname.notificacion.get: config.pathname.notificacion.put + $id.val(), datos, beforeSend, HTTPMethod, true, 'application/x-www-form-urlencoded; charset=UTF-8', 'json', statusCode, true, headers);
        ...
        resquest.done(function (result, textStatus, jqXHR) {
            ...
            $dataTable.row.add([
                ...
                '<button type="button" class="btn btn-sm btn-icon btn-pure btn-outline delete-row-btn" data-toggle="tooltip" data-original-title="Delete" data-href="' + config.pathname.notificacion.delete + result.id + '"><i class="ti-close" aria-hidden="true"></i></button>'
                        + '<button type="button" class="btn btn-sm btn-icon btn-pure btn-outline update-row-btn" data-toggle="tooltip" data-original-title="Modificar" '
                                + 'data-id="' + result.id + '"'
