 ...
 *    @file ~/freelife/public/js/categoria/categoria.js
...
var Categoria = function () {
    ...
    //cache DOM
    var $layout = $('div.categoria');
    var $menu = $("#menu-categoria");
    var $form = $layout.find('form');
//    var $agregar = $layout.find('button[type="submit"]');
    var $cancelar = $form.find('button[type="reset"]');
        ...
        fields: {
            ...
            descripcion: {
                ...
                validators: {
                    ...
                    stringLength: {
                        message: 'La la descripción de la categoría debe tener máximo 50 carateres'
                    },
                    blank: {}
                }
            },
            ...
            if (result.data.length > 0) {
                ...
                for (var key in result.data) {
                    ...
                    $dataTable.row.add([
                        ...
                        '<button type="button" class="btn btn-sm btn-icon btn-pure btn-outline delete-row-btn" data-toggle="tooltip" data-original-title="Delete" data-href="' + config.pathname.categoria.delete + result.data[key].id + '"><i class="ti-close" aria-hidden="true"></i></button>'
...
//        var $form = $(event.target).parents("form");
        ...
        var resquest = ajax($id.val() === '' ? config.pathname.categoria.post : config.pathname.categoria.put + $id.val(), datos, beforeSend, HTTPMethod, true, 'application/x-www-form-urlencoded; charset=UTF-8', 'json', statusCode, true, headers);
        ...
        resquest.done(function (result, textStatus, jqXHR) {
            ...
            $dataTable.row.add([
                ...
                '<button type="button" class="btn btn-sm btn-icon btn-pure btn-outline delete-row-btn" data-toggle="tooltip" data-original-title="Delete" data-href="' + config.pathname.categoria.delete + result.id + '"><i class="ti-close" aria-hidden="true"></i></button>'
                        + '<button type="button" class="btn btn-sm btn-icon btn-pure btn-outline update-row-btn" data-toggle="tooltip" data-original-title="Modificar" '
                        + 'data-id="' + result.id + '"'
