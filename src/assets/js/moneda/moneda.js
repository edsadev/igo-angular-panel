 ...
 *    @file ~/freelife/public/js/moneda/moneda.js
...
var Moneda = function () {
    ...
    //cache DOM
    var $layout = $('div.moneda');
    var $menu = $("#menu-moneda");
    ...
    $form.formValidation({
        ...
        fields: {
            moneda_iso: {
                ...
                validators: {
                    notEmpty: {
                        message: 'Introduzca el código ISO de la moneda'
                    },
                    stringLength: {
                        message: 'El código ISO de la moneda debe tener máximo 50 carateres'
                    },
                    blank: {}
                }
            },
            descripcion: {
                ...
                validators: {
                    notEmpty: {
                        message: 'Introduzca la descripción de la moneda'
                    },
                    stringLength: {
                        message: 'La descripción de la moneda debe tener máximo 50 carateres'
                    },
                    blank: {}
                }
            },
...
//        var $form = $agregar.parents("form");
    ...
    function _get(event) {
        ...
        var resquest = ajax(config.pathname.moneda.get, datos, beforeSend, HTTPMethod, true, 'application/x-www-form-urlencoded; charset=UTF-8', 'json', statusCode, true, headers);
            ...
            if (result.data.length > 0) {
                ...
                for (var key in result.data) {
                    ...
                    $dataTable.row.add([
                        ...
                        '<button type="button" class="btn btn-sm btn-icon btn-pure btn-outline delete-row-btn" data-toggle="tooltip" data-original-title="Delete" data-href="' + config.pathname.moneda.delete + result.data[key].id + '"><i class="ti-close" aria-hidden="true"></i></button>'
                                ...
                                + 'data-moneda_iso="' + result.data[key].moneda_iso + '"'
...
//        var $form = $(event.target).parents("form");
        ...
        var resquest = ajax( $id.val() === '' ? config.pathname.moneda.post: config.pathname.moneda.put + $id.val(), datos, beforeSend, HTTPMethod, true, 'application/x-www-form-urlencoded; charset=UTF-8', 'json', statusCode, true, headers);
        ...
        resquest.done(function (result, textStatus, jqXHR) {
            ...
            $dataTable.row.add([
                ...
                '<button type="button" class="btn btn-sm btn-icon btn-pure btn-outline delete-row-btn" data-toggle="tooltip" data-original-title="Delete" data-href="' + config.pathname.moneda.delete + result.id + '"><i class="ti-close" aria-hidden="true"></i></button>'
                        + '<button type="button" class="btn btn-sm btn-icon btn-pure btn-outline update-row-btn" data-toggle="tooltip" data-original-title="Modificar" '
                                + 'data-id="' + result.id + '"'
                                + 'data-moneda_iso="' + result.moneda_iso + '"'
